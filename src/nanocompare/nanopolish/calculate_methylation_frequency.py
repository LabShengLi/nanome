...
parser.add_argument('-n', '--num_motifs', type=str, required=False, default="num_motifs")
args, input_files = parser.parse_known_args()
assert (args.call_threshold is not None)

num_motifs_str = args.num_motifs
...
for f in input_files:
    ...
    for record in csv_reader:
        ...
        try:
            num_sites = int(record[num_motifs_str])
        except:
            print(f"Encounter error:{record}")
            continue
