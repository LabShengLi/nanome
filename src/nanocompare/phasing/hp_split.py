...
def parse_arguments():
    ...
    parser.add_argument('--num-class', type=int, help="number of class for input file, 2 for 5mc/5c, 3 for 5c/5mc/5hmc",
                        default=3)
...
if __name__ == '__main__':
    ...
    for hp_type in hpdf['haplotype'].unique():
        ...
        outfn = os.path.join(outdir, f'{args.dsname}_{args.tool.lower()}_read_pred_{args.region}_{hp_type}.tsv.gz')
        to_read_preds_file(predDict, outfn)
        logger.info(f"save to {outfn}")

        outfn = os.path.join(outdir, f'{args.dsname}_{args.tool.lower()}_site_freq_{args.region}_{hp_type}.tsv.gz')
