...
import pandas as pd
import csv
from sklearn.metrics import roc_curve, precision_recall_curve, auc, classification_report, confusion_matrix, average_precision_score
...
def importGroundTruth_coverage_output_from_Bismark(infileName, chr_col=0, start_col=1, meth_col=3, meth_reads_col=4, unmeth_reads_col=5, covCutt=10, baseCount=1, chrFilter=False, gzippedInput=True):
    ...
    for row in infile:
        ...
        if chrFilter == False or chrFilter == tmp[chr_col]:
            if (int(tmp[meth_reads_col]) + int(tmp[unmeth_reads_col])) >= covCutt:
                key = "{}\t{}\t{}\n".format(tmp[chr_col], start, start)
                if key not in cpgDict:
                    cpgDict[key] = float(tmp[meth_col]) / 100.0
                else:
    ...
    infile.close()
    print("###\timportGroundTruth_coverage_output_from_Bismark: loaded information for {} CpGs".format(len(cpgDict)))
    return cpgDict
...
if __name__ == '__main__':
    ...
    if argv[7] == "encode":
        ...
        print("UniversalMethStatsEvaluation.standalone_01.py ERROR: Unknown bacground truth parser configuration. Aborting. FYI: currently supported are: encode, oxBS_sudo, bismark")
        exit()
    ...
    print("Data points for all stats:", len(DeepSignal_Tombo_Nanopolish_DeepMod_Bacground))
    ...
    print("Data points for correlation:", len(DeepSignal_Tombo_Nanopolish_DeepMod_Bacground_corr))
