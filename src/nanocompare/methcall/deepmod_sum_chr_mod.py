"""
We modified the DeepMod sum_chr_mod file to extract not only chr1-23
"""
import glob
...
def get_all_chrs(pred_folder, baseOfin):
    allbedfiles = glob.glob(os.path.join(pred_folder, (f"*/*/*/mod_pos.*.{baseOfin}.bed")))
    allbedfiles.extend(glob.glob(os.path.join(pred_folder, (f"*/*/mod_pos.*.{baseOfin}.bed"))))
    allbedfiles.extend(glob.glob(os.path.join(pred_folder, (f"*/mod_pos.*.{baseOfin}.bed"))))

    chrkeys = []
    for fn in allbedfiles:
        basefn = os.path.basename(fn)
        chrname = basefn.replace('mod_pos.', '').replace(f'.{baseOfin}.bed', '')[:-1]
        chrkeys.append(chrname)
    return list(set(chrkeys))
...
def sum_amod_handler(run_Q):
    ...
    while not run_Q.empty():
        ...
        allbedfiles = glob.glob(os.path.join(pred_folder, ("*/*/*/mod_pos.%s-.%s.bed" % (ck, baseOfin))))
        allbedfiles.extend(glob.glob(os.path.join(pred_folder, ("*/*/mod_pos.%s-.%s.bed" % (ck, baseOfin)))))
        allbedfiles.extend(glob.glob(os.path.join(pred_folder, ("*/mod_pos.%s-.%s.bed" % (ck, baseOfin)))))
