...
if [ "$dsname" == "NA19240" ] || [ "$dsname" == "APL" ]; then
    BSLabel="WGBS"
else
    BSLabel="RRBS"
fi
...
if [[ " ${actions} " =~ " compute " ]]; then
    ...
    computeMatrix reference-point -p 16 \
        ...
        --binSize $binSize \
        --skipZeros \
        -o $outdir/$dsname.bin${binSize}.mat.gz
fi

#### Profile plot:
plotProfile -m $outdir/$dsname.bin${binSize}.mat.gz \
    -out $outdir/$dsname.bin${binSize}.profileplot.png \
    --plotType=lines \
    --perGroup \
...
plotHeatmap --colorMap Reds \
    ...
    -m $outdir/$dsname.bin${binSize}.mat.gz \
    --whatToShow 'plot, heatmap and colorbar' \
    --refPointLabel "TSS" \
    --xAxisLabel "dist. to TSS" \
    --outFileSortedRegions $outdir/$dsname.bin${binSize}.sortedRegions.txt \
    --outFileNameMatrix $outdir/$dsname.bin${binSize}.outMatrix.tsv.gz \
    --yMin 0 \
    --yMax 1 \
    -out $outdir/$dsname.bin${binSize}.plotheatmap.png
